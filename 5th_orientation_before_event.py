...
# ### calculate orientation duration mean between stim and event
...
# ### plot 15 min plot for orientation duration mean
...
# ### calculate orientation duration ratio to total in LH, location + ori in each frame
    ...
    for j in event:        
        ...
        plot=sns.pointplot(data=t, x="min_15bin", y="loca_ori_duration", hue="loca_ori",  
                           hue_order=["screenside_oriented", "screenside_blind", 
                                      "rewardside_oriented", "rewardside_blind"], palette=[p, p, p, p], 
                           linestyles=['-', '-.', ':', '--'],
                           markers=["o", "x","D", "*"], capsize=.01, errorbar=('ci', 68)) #
        plt.setp(plot.collections, sizes=[70])
        plt.setp(plot, linewidth=1)
        plot.set(xlabel="")
        plot.set(ylabel="duration(s)")
        plt.title(title)
        plt.xticks(rotation = 15, ha='right')
...
for i, p in zip(gander, color_list) :
    for j in event:        
        ...
        plot=sns.pointplot(data=t, x="min_15bin", y="ori_index", hue="event_type",  hue_order=["hit", "miss", "mistake", "corr_reject"], palette=[p, p, p, p], 
                           linestyles=['-', '-.', ':', '--'], markers=["o", "x","D", "*"], capsize=.01, errorbar=('ci', 68))
        plot.set(xlabel="")
...
# ##### Combine plot event ori index into one
